...
const ActivityTokenCell: React.FC<ActivityTokenCellProps> = ({
    ...
    return (
        <div className="flex col-span-3 lg:col-span-2 items-center font-archivo text-sm md:text-md">
            <Link to={path} className="rounded-lg overflow-hidden hover:opacity-60 min-w-[60px]">
                <Img src={media || ''}
                     className="object-contain w-[60px] h-[60px]"
                     alt={title}
                     loader={<RectLoader width={60} height={60}/>}
                     unloader={
                         <div className="flex items-center justify-center min-w-[60px] min-h-[60px] rounded-lg">
                             <img src={brokenImage} alt="not found" className="object-contain w-[40px] h-[40px]"/>
                         </div>
                     }
                />
            </Link>
            <div className="flex flex-col pl-4 truncate">
                <Link to={path} className="font-bold truncate hover:underline">
                    {title}
                </Link>
